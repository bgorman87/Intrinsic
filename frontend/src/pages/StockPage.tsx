...
const StockPage = ({ theme, switchTheme }: Props) => {
  ...
  const calculateEPS = (netIncome: number | null, sharesOutstanding: number | null) =>
  ...
  const calculateEnterpriseValue = (marketCap: number | null, debt: number | null, cash: number | null) =>
    marketCap && debt !== null && cash !== null ? marketCap + debt - cash : null;

  const calculateBookValuePerShare = (assets: number | null, liabilities: number | null, sharesOutstanding: number | null) =>
    assets && liabilities && sharesOutstanding ? (assets - liabilities) / sharesOutstanding : null;

  const calculateReturnOnAssets = (netIncome: number | null, assets: number | null) =>
    netIncome && assets ? netIncome / assets : null;

  const calculateCurrentRatio = (assets: number | null, liabilities: number | null) =>
  ...
  const calculateDividendYield = (trailingDividend: number | null, current: number | null) =>
    trailingDividend && current ? (trailingDividend / current) * 100 : null;

  const calculateCashFlowYield = (fcf: number | null, marketCap: number | null) =>
  ...
  const enterpriseValue = calculateEnterpriseValue(stock.marketCap, stock.debt, stock.cashRawEq);
  const bookValuePerShare = calculateBookValuePerShare(stock.assets, stock.liabilities, stock.sharesOutstandingRaw);
  ...
  const debtToEquity = calculateDebtToEquity(stock.debt, stock.stockholdersEquityRaw);
  const dividendYield = calculateDividendYield(stock.trailingDividendRateRaw, stock.current);
  const cashFlowYield = calculateCashFlowYield(stock.fcfRawValue, stock.marketCap);
        ...
        <div className="stock-page__content__details">
          <div className="stock-page__intrinsic-values">
            <p
              ...
              data-stock={stock.pe && stock.current ? stock.pe > stock.current : "N/A"}
            >
              ${stock.pe?.toFixed(2)}
            </p>
            <p
              className="stock-list__intrinsic__value roe"
              data-stock={stock.roe && stock.current ? stock.roe > stock.current : "N/A"}
            >
              ${stock.roe?.toFixed(2)}
            </p>
            <p
              className="stock-list__intrinsic__value dcf"
              data-stock={stock.dcf && stock.current ? stock.dcf > stock.current : "N/A"}
            >
              ${stock.dcf?.toFixed(2)}
            </p>
          </div>
        ...
        <div className="stock-page__content__metrics__container">
          ...
          <div className="stock-page__content__metrics">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatDollar(stock.marketCap)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Growth Estimate (5 yr)</p>
              <p className="metric-quantity">{formatPercent(stock.growthEstimate)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Shares Outstanding</p>
              <p className="metric-quantity">{formatNumber(stock.sharesOutstandingRaw)}</p>
            </div>
            <div className="stock-page__content__metric">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatDollar(enterpriseValue)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Book Value Per Share</p>
              <p className="metric-quantity">{formatDollar(bookValuePerShare)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">ROA (Return on Assets)</p>
              <p className="metric-quantity">{formatPercent(returnOnAssets)}</p>
            </div>
            <div className="stock-page__content__metric">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatPercent(dividendYield)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Cash Flow Yield</p>
              <p className="metric-quantity">{formatPercent(cashFlowYield)}</p>
            </div>
            <div className="stock-page__content__metric">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatNumber(stock.esgScore)}</p>
            </div>
          </div>
          <div className="stock-page__content__metrics">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatDollar(stock.netIncome)}</p>
            </div>
            <div className="stock-page__content__metric">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatDollar(stock.liabilities)}</p>
            </div>
            <div className="stock-page__content__metric">
            ...
            <div className="stock-page__content__metric">
              ...
              <p className="metric-quantity">{formatDollar(stock.longTermDebt)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Cash & Equivalents</p>
              <p className="metric-quantity">{formatDollar(stock.cashRawEq)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Free Cash Flow</p>
              <p className="metric-quantity">{formatDollar(stock.fcfRawValue)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Stockholders Equity</p>
              <p className="metric-quantity">{formatDollar(stock.stockholdersEquityRaw)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Historical ROE</p>
              <p className="metric-quantity">{formatPercent(stock.historicalROE)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Trailing Dividend Rate</p>
              <p className="metric-quantity">{formatDollar(stock.trailingDividendRateRaw)}</p>
            </div>
            <div className="stock-page__content__metric">
              <p className="metric-title">Controversy Level</p>
              <p className="metric-quantity">{formatNumber(stock.controversy)}</p>
            </div>
            </div>
            </div>
        </div>
